...
The [RuntimeClient](/docs/api/client) is the primary consumer API. It exposes Promise-based methods (`render()`, `export()`, `connect()`) and event subscription (`on('log')`, `on('progress')`). It does not know about workers or message protocols; it delegates to `RuntimeWorkerClient`, which wraps the transport. The client lazily initializes the worker and transport on first `connect()` or `render()`.

### Transport Layer (MessagePort)

The [RuntimeTransport](/docs/api/transport) is a low-level interface: `send(message, transferables?)` and `onMessage(handler)`. The default implementation uses a Web Worker and `postMessage`. The protocol is defined by `RuntimeCommand` and `RuntimeResponse` types. This abstraction allows swapping in WebSocket, HTTP, or native FFI without changing the client or framework.
...
1. **Client** receives `render()`, calls `ensureConnected()` if needed, then `RuntimeWorkerClient.render()`.
...
- [API: Client](/docs/api/client) — `createRuntimeClient` and `RuntimeClient` types
